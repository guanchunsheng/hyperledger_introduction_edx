...
第一章介绍分布式账本的区块构建，以及区块链的构建。这一章是本教程的基础，为深入的理解后续章节打下基础。需要通过本章的学习可以提高对区块链知识的理解。
...
大型计算机是高度中心化的。它们通常集中了所有的计算能力、内存、数据存储和代码。访问大型计算机主要通过”哑终端“来进行，只包含输入和输出，终端并不存储和处理数据。

随着个人计算机和私有网络的兴起，现在在服务器和客户端上都具备相似的计算能力。这在某种程度上催生了“客户机-服务器”体系结构，它支持关系型数据库系统的开发。在大型计算机上维护的海量数据集，现在可以转移到分布式的结构上。数据可以在服务器之间复制，数据的子集可以在客户端上访问和处理，然后再同步回服务器上 。

随着时代的发展，现在互联网和云计算架构支持各种各样的计算设备，而大型机主要针对大型企业和政府的需求。尽管这种“云架构”在硬件方面是分散的，但它产生了应用程序本身的集中化，比如脸书，推特和谷歌等。

目前，我们正经历从集中式的计算、存储和处理向分布式体系结构和系统的转变。

根据[Muneeb Ali](https://medium.com/@muneeb/the-next-wave-of-computing-743295b4bc73) 所说，这些系统的目标是：

> “将数字资产的控制交给终端用户，无须相信任何第三方服务器和基础设施”

分布式账本技术是促成这个转变的关键性创新。

## 1.4 分布式账本技术（DLT）

**分布式账本**是一类数据结构，分布在多个计算设备之间，通常在不同的地理位置和区域。

**分布式账本技术**包括区块链技术和智能合约。分布式账本技术先于比特币就已经存在，比特币将时间戳、P2P网络、密码学和共享算力、一种新的共识算法，与分布式账本结合在了一起。

总的来说，分布式账本技术包含3个组成要素：

* **数据模型** - 获取账本的当前状态
* **业务语言** - 改变账本状态
* **协议** - 在参与方之间，就账本是否要接受相关业务，以及按照什么顺序记录，达成共识

---
Robert Schwentker
> **什么是区块链技术？**
>
> 区块链是一个P2P的分布式账本，通过共识机制、结合智能合约系统和其他辅助性的技术组成。
>
> 这些东西在一起可以构成新一代的交易应用，它可以建立信任、可问责性和核心透明性，这将简化业务流程和法律约束。
>
> 所有的分布式账本都有第一个区块，或者叫创世区块。每个区块都会包含一个或者多个交易。连接到一个区块链就相当于相关的人，通过应用，连接到这个分布式账本。
>
> 所以钱包就是一个例子。
>
> 一个人可以将数字资产比如密码学货币转账给另一个人，这样资产就从一个人的钱包转移到了另一个人的钱包，然后这个交易就被记录到区块链里。之后这笔分布式账本交易通过P2P网络广播给全网，并没有中介机构比如银行或者支付公司来处理这笔交易。
>
> **你能给出一个近年来实际中有使用的区块链产品么？**
>
> 当然，区块链就是通过比特币被大家熟知的。比特币区块链从2009年开始就存在了。比特币是一种密码学货币，但有趣的是，人们混淆了两个概念，比特币不等于区块链，区块链也不等于比特币。
>
> 区块链是一种分布式的账本，追踪各种资产，跟密码学货币比如比特币是不同的。跟资产相关的这些交易被打包到区块中，每个区块可以包含任意数量的交易。区块链网络中的节点或者机器将这些交易打包并且通过网络散播出去。
>
> **你已经提到了区块链在P2P节点中是怎么处理的，那么它们是怎么同步起来的呢？**
>
> 区块链同步是通过共识的概念完成的，共识是在网络节点之间达成一致的意思。所以最终，网络中的每一个机器都获得了一份相同的区块链拷贝。
---
...
**智能合约** - 在系统内，当满足特定条件时，会自动执行的预先定义好的计算机程序

**共识**- 指的是保证参与方都承认，系统的某个状态是真实有效的

**区块链**是分布式账本技术的一个特别的术语或子集，由有序的区块链接而成，所以叫“区块链”。

**区块**表示捆绑在一起的一组交易，它们在同一时间加入到链中。在比特币区块链中，矿工节点打包未确认但是有效的交易到一个块中。每个块包含一定数量的交易。在比特币网络中，矿工必须解出一个密码学的难题来获得下一个块的记录权。这个过程称为“工作量证明 PoW”，需要很大的计算能力。我们将在共识算法章节讨论工作量证明的问题。区块链技术的简明历史，请参考[这里](https://hbr.org/2017/02/a-brief-history-of-blockchain)。

时间戳是区块链技术的另一个关键特性。每个区块都有时间戳，新的区块都指向前一个区块。结合密码学的**散列（哈希/Hash）**，这样的带有时间戳的链接在一起的区块，从创世区块开始的，无法修改已经写入的交易记录。
...
> “Merkle树用来汇总一个区块中所有的交易，对于所有交易组成的集合生成一个整体的数字指纹，这对于验证区块中是否包含的特定的交易非常高效”

![](/assets/Bitcoin_Block_Data.png)
...
> **分布式账本技术和区块链的区别是什么呢？**
>
> 分布式账本和区块链的区别在外界看来非常晦涩。对我个人来说，分布式账本是一个非常好的，非常特别的方式来描述分布式的数据库。在这个系统中，你、我和其中的每个人都同时保有一系列交易的拷贝。
>
> 这样的数据结构就叫区块链，但是现在大家都用区块链这个词代表密码学货币，企业中部署的DLT等。
>
...
> **那么，什么是区块链呢？**
>
> 作为超级账本的安全专家，关于区块链我有一些不同角度的看法。
>
> 我最初是一个开发者，并且会用一种对抗的方式去看世界。通常是我的软件和真实世界的对抗。
>
> 对我来说，区块链无疑为解决困扰我们许多年的问题提供了另一个解决它们的可能性。它可以通过很多方式在分散开的各部分之间建立分布式共识，允许我们每时每刻建立一个唯一的真实状态。
>
> 过去，我们总是被“一天的时间中到底发生了什么”这样的问题困扰.
>
> 银行核对账簿，库存系统清理销售和交货记录…
>
> 通过区块链，我们实时的记录事务，这为我们提供了一种即时记录世界状态的方法。
>
> 所以，很多旧系统都可以滚蛋了，不再需要这么多的系统管理的工作量了。我们再也不用在夜里批处理任务了。
>
> 取而代之的是，我们可以实时验证，我们总是可以知道世界到底处于什么状态。
>
> 这为应用的部署提供了一个新的方式，比如如果我们明确的知道螺栓在哪里，那么是不是需要他们的时候提前2分钟确定他们还在那里就行了，就像精益制造一样。
>
> 它也可以应用到所有我们需要追踪物料和人工、资金等等的领域。
>
...
>
> 从安全的角度看，区块链不是银弹。它们不会替代现有的业务逻辑。更准确的说他们是日志系统，就在事务发生当时记录下发生了什么，而不是从顾客口袋里掏钱的工具。
>
> 它不是软件类型的业务流程，因为它是日志，它是真实，因为很多其他业务逻辑将会建立在它之上，所以它的安全性跟现有业务逻辑没有区别。
>
...
> **那么Sawtooth呢，不是这样的吧？**
>
> Sawtooth的网络更像是一个星形模型。可以选择一部分节点，将你的节点连接进去，然后只要节点连接没问题，那么你可以最终和每个节点沟通，通过1跳，2跳或者3跳等等，根据你的网络的规模。
>
...
区块链可以是开放的（permissionless，比如比特币和以太坊），也可以是封闭的（permissioned，比如各种超级账本框架）。开放的区块链也叫公链，因为任何人都可以加入网络。封闭的区块链，也叫私链，需要提前验证网络中的参与者，而且这些参与者一般是互相知道的。

是选择公链还是私链取决于具体业务。大多数的公司场景，在彼此同意展开业务之前，需要进行审核。不同业务需要进行信息交换的一个例子是供应链管理。供应链管理是一个典型的封闭区块链用例。你肯定不会想要未经审核的公司参与到网络中来。参与进来的每一方，在区块链上执行交易之前，都需要经过授权。这些交易能够让其他公司了解到供应链中具体某个东西现处何地。
...
开放的P2P系统不需要一定数量的peer保持在线，一般来说是比较慢的。封闭的P2P网络要保证一定的在线和通信链路的质量。

![](/assets/Network_Architectures.png)

---
Robert Schwentker
>**什么是P2P网络，peer是怎么对区块链上的东西达成一致的呢？**
>
> 区块链网络就是一组计算机，按照P2P结构组织在一起的。共识是peers就区块链上的数据进行同步的过程。有一些共识机制和算法。一个是工作量证明，一个是权益证明。还有时间流逝证明，简化拜占庭容错等等。
>
...
区块链中数据的不变性可能是区块链方案能够替代传统的中央服务器方案的优势所在，其上记录着各种社会经济数据。永久性，或者说不变性，使得区块链对于会计，金融交易，身份管理和资产所有权，资产管理和转移都具有很大的作用。一旦一个交易写进了区块链，那么谁都不能修改，至少是非常难以修改。
...
> **什么是智能合约？**
>
> 早在1996年，一个叫Nick Szab的人就提出了这个词“智能合约”。你可以这么理解这个词，它是一种计算机协议，可以用来促进、验证和执行一个法律合同。智能合约是一个描述计算机代码的词。
>
...
2008年P2P现金系统被发明出来，大家都叫他比特币。比特币是一个全球的分布式支付网络，使用分布式的开放公有的基础设置，也就是“permissionless”的系统。这是一个分布式计算的非常有说服力的杀手级应用。大家可以在世界上任何地方发出和接收比特币，完全是P2P的方式，不需要经过一个受信任的第三方，比如银行。
...
> **为什么比特币和以太坊会出现呢？他们要解决什么问题？**
>
> 比特币首先在2009年1月份出现，适逢当时的全球金融危机。这个系统的一个主要动因是可以通过互联网传递价值，而不需要中介。因此它的一个最大的创新和解决的问题就是“双重支付”问题。
>
> 所以事实上，比特币就是可编程的货币。
>
> 至于以太坊，是对应着比特币诞生的。比特币专注于互相传递资金，所以它的编程语言很有限。以太坊使用了更丰富的编程语言和工具，以便用于更多其他类型的程序和应用。
>
> 以太坊的核心创新是EVM，也就是以太坊虚拟机。EVM运行于以太坊网络上，是图灵完备的软件。Vitalik Buterin起草了以太坊白皮书。它的一些关键特性包括数据不变性，未经授的用户不能改变数据。
>
...
据[Vitalik Buterin](https://www.youtube.com/watch?v=pKqdjaH1dRo)所说:
> “密码经济学构建了具有某些特性的系统，它使用密码学证明过去发生的种种都真实无疑，使用系统内的经济激励来保证系统的存续”
...
> **你能给出另一个产生区块链的系统的例子么？比如以太坊，它跟比特币有什么区别？**
>
> 可以。以太坊也是公链。它也是将交易排序和打包进区块中。但是，以太坊可以定义成一个开源平台，让开发者可以构建和部署智能合约和分布式应用，分布式应用也叫Dapp。
>
...
既然它们是密码学安全的，那么Dapps可以称为“安全应用”。一些比较突出的Dapps包括：
...
让我们开始检查一下比特币和以太坊区块链，这两个都是开放的公链。我们将会从几个大的交易和创世区块上进行检查。我们会看看区块高度，交易时间，矿池，时间戳和区块奖励。
...
没有挖矿过程，因为一开始所有的货币就都已经存在了。简单的说，随机选择节点去检验这个区块，这个随机选择的概率根据节点持有的股权来决定。如果节点X有2个币，节点Y有1个币，那么在验证交易的时候X被选中的概率是Y的2倍。
...
基本思想是单一的验证者将提出的交易打包形成一个新的块。注意，跟比特币不同，验证者是已知的，因为这是一个封闭的账本环境。
...
一个实际的例子是![ByzCoin](http://hackingdistributed.com/2016/08/04/byzcoin/)，力图对原始的比特币做出关键的改进。针对弹性和高延迟的问题，ByzCoin交易不可逆的提交到区块链的时间是秒级的。附加的优点是通信树，“优化了一般情况下的交易提交和验证”（![2016 Kudelski安报告](https://www.kudelskisecurity.com/sites/default/files/files/kudelski_Security_blockchain_20161213.pdf)）

### 1.18.5 权威证明

PoA可以用于有权限限制的账本中。它使用了一组“权威”，是一组指定的节点，可以用来创建新的区块，保证账本安全。使用PoA的账本，需要多数权威签字来创建区块。

### 1.18.6 标准PoW和有权限限制的共识方式比较
...
> **相对于其他的permissioned账本，超级账本有什么独特性呢？**
>
> 在开源领域的各种区块链技术中，超级账本确实是一个独一无二的社区。
>
> 我们自己定位是有点像下一个Apache，类似于其那些社区中的社区。
>
...
> **能举几个其他的区块链和分布式账本技术系统的例子么？他们的优点是什么？**
>
> chain.com的Chain Core，是为了金融业务设计的，以及安全性，债券和现金业务。他们的公司同Visa，Citigroup和纳斯达克的关系都很紧密。
>
...
更多信息参考： [https://www.corda.net](https://www.corda.net)

#### Quorum

JP摩根开发的Quorum实际上是以太坊公链的一个分叉，使用基于投票的共识算法来搭建企业级的分布式账本和智能合约平台。在网络内部基于need-to-know方式实现数据隐私。
...
对于所有参与者来说，区块链上交易的监管缺失都会造成一个不确定的环境。高度依赖监管的行业，比如金融服务在DLT领域需要审慎处理。美国国家证券交易委员会最近澄清了他们对于ICO的立场。中国政府实际上禁止了所有的ICO，60个主要的ICO平台都受到了调查。
